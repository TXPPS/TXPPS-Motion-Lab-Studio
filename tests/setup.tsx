...
function createEngineStub() {
  ...
  const stub = {
    ...
    stopAudition: vi.fn(),
    inputLevel: vi.fn(() => 0),
    // Open and audible are two different questions — see `InputTap` in the
    // engine. A stub that answered only one of them let a component read
    // `isInputOpen` off an object that did not have it.
    isMonitoring: vi.fn(() => false),
    isInputOpen: vi.fn(() => false),
    openInput: vi.fn(() => Promise.resolve(true)),
    setInputAudible: vi.fn(),
    closeInput: vi.fn(),
    closeAllInputs: vi.fn(),
    monitoringCount: vi.fn(() => 0),
    latency: vi.fn((): { base: number; output: number; total: number } | null => null),
    previewClip: vi.fn(() => Promise.resolve(true)),
    stopPreview: vi.fn(),
    setMonitorCue: vi.fn(),
    motionWaveFrameOf: vi.fn((): Float64Array | null => null),
    canChooseOutput: vi.fn(() => false),
