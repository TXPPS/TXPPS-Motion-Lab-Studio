...
{
  const ceiling = await page.evaluate(async (budget) => {
    ...
    for (let i = 0; i < 400; i += 1) {
      ...
      if (!id) break;
      added += 1;
      for (let k = 0; k < 3; k += 1) if (st().addEffect(id, kinds[k % kinds.length])) inserts += 1;
      ...
      if (i % 4 === 3) {
        ...
        // bimodal.
        if (p90 > budget && (await sample()) > budget) {
          return { tracks: total, inserts, p90, marks, hitCap: false };
        }
      }
    }
    return { tracks: started + added, inserts, p90: await sample(), marks, hitCap: true };
  }, FRAME_MS * 2);
  ...
  record(
    ...
    'tracks',
    ceiling.hitCap
      ? `never exceeded ${(FRAME_MS * 2).toFixed(0)} ms p90 — the ceiling is above this`
...
if (!JSON_ONLY) console.log('\nTransport fuzz');
{
  const fuzz = await page.evaluate(async () => {
    ...
    while (performance.now() - t0 < 6000) {
      ...
      else if (r < 0.56) await engine.play();
  ...
  else record('time to quiescence', fuzz.settleMs, 'ms', '0 sources, 0 held');
}
...
if (!JSON_ONLY) console.log('\nStuck-note fuzz');
{
  const notes = await page.evaluate(async () => {
    ...
    return {
      ...
      covered: Object.keys(held).length,
