...
| RA-014     | P2       | 313 distinct sub-44 px targets beyond the named tickets. `e2e/devicewindow.spec.ts` fails on three of them — the console's power (5 px), name (12 px) and options (11 px) controls against its 44 px assertion. Eleven pixels also fails WCAG 2.2's 24 px _pointer_ minimum, so lowering the assertion is not the answer; growing the target grows every device row on every channel, which is a decision about how the console looks.           |
