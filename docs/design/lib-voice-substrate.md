...
```
    Linear:          f(x) = x
    Exponential:     f(x) = 1 − e^(−k·x) + e^(−k)·x                            (3)
    TargetSeeking:   f(x) = (1 − e^(−k·x)) / (1 − e^(−k))                      (4)
```

(3) carries a term that forces the segment to actually reach its target at
...
**(3) is a correction, and the version it replaces is worth keeping on the page.**
It was written here as

```
    f(x) = (1 − e^(−k·x) − e^(−k)·(1 − x)) / (1 − e^(−k))
```

which is 1 at `x = 1` — the property the paragraph above claims, and the one
anybody checking it would check. It is `−e^(−k)/(1 − e^(−k))` at `x = 0`, about
−3.1 % at `k = 3.5`. The numerator carries the offset _and_ the whole expression
is divided by the same quantity, so the two corrections agree only at the far
end: every segment starts a step away from where the one before it finished, and
every segment boundary is a click. Adding `e^(−k)` back to the numerator and
dropping the division gives the form above, which is 0 at 0 and 1 at 1 by
inspection and monotonic for every `k > 0`, since `f'(x) = k·e^(−k·x) + e^(−k)`.

This is VS-02's shape one level down and it is the third instance in this
document: **the consequence the prose states is true, and a consequence nobody
wrote down is not.** `envelope_tests.cpp` asserts that no segment boundary moves
further in one sample than the segment itself does, for all three curves, and the
old formula fails that case and only that case.

(4) is written here as a closed function of position rather than as the
incremental `L[n] = L[n−1] + (ceiling − L[n−1])·g` it was, and the two are the
same curve: the recursion approaches a ceiling and the closed form renormalises
so the segment arrives at the end of the time constants it was given. The closed
form is what makes `L(1.0) = 1.000` exact rather than nearly, which is what VS-11
measures.
...
**A row that names its own mutation has to be able to catch it, and two here
could not.** VS-02 was written to fail when the partition is replaced by a flag
per voice, and the criteria it stated — every `VoiceId` exactly once, no
duplicates, `liveCount()` equal to the allocated span — are all still true under
that mutation: the array is never reordered, so it stays a permutation, and the
count agrees with the span because both are read from the same counter. What the
flag breaks is _disjointness of ownership_, and nothing asked about it. VS-23 was
the same shape from the other end: its stated criteria are satisfied by a
receiver that **ignores** a zone message it cannot fit, which is the outcome the
row's own prose says must not pass.

Both are corrected below and both corrections are the same move — replace a
consequence stated in prose with the observable that distinguishes it. This is
the cell-title failure at spec level: a row that tests what its title implies
rather than what its criteria say. The rest of §8 was swept for it; every other
named mutation and every other "an implementation that does X fails" clause is
reachable from the numbers beside it.

The sweep found a third instance outside §8, in §5.2's formula (3), and it is the
same sentence again: the formula reached its target at `x = 1`, which is what its
paragraph claims, and started 3 % away from where the previous segment ended,
which nothing said. It is corrected there with the version it replaced kept on
the page.
...
| VS-02 | **Partition integrity.** After 100 000 randomised allocate/release/retire/steal operations.                                                                     | Walk the permutation, and ask `ownerOf()` about every slot.                | Every `VoiceId` appears **exactly once** across free ∪ allocated; `liveCount()` equals the allocated span; **zero** duplicates. **And the two spans agree with ownership**: every voice in the allocated span has an owner, every voice in the free span has none. Mutation: replace the partition with a boolean flag per voice. The array is then never reordered, so the first three clauses hold under it — the last one is what fails, and a voice that has an owner while sitting in the free span is one the next allocation hands out while it is still sounding. |
...
| VS-23 | **Zone truncation.** Lower zone 10 members, then upper zone 10 members.                                                                                         | Both zones' channel sets, by size and by channel.                          | Lower holds **exactly 4** members (channels 2–5), upper **exactly 10** (15 down to 6), total **exactly 14**, and the two sets are **disjoint**. The counts are what separate the three implementations: recomputing only the addressed zone gives 10 and 10 and fails disjointness; _ignoring_ a message that will not fit gives 10 and 0, which is disjoint and under 14 and was passing the criteria this row used to state.                                                                                                                                            |
