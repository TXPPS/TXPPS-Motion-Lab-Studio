...
**MotionLab does:** Both are selectable, on a browser that allows it. Input is
`Track.inputDeviceId` per track; output is the `outputDeviceId` preference
(`src/state/prefsStore.ts`), chosen in `src/components/settings/AudioSetup.tsx`
and applied by `Engine.applyOutputDevice` (`src/audio/engine.ts`), which calls
`AudioContext.setSinkId` where it exists and otherwise says so once through the
diagnostics log rather than failing quietly. `Engine.canChooseOutput()` reports
whether the browser offers the choice at all.

This entry recorded the opposite, as a **P0**, and it was written when it was
true. It stopped being true and nothing said so — which is the failure
`scripts/parity-guard.mjs` exists to make loud, and this is the entry its first
full run caught. It reads `src/` for every symbol a chapter says is absent; two
of the six named here are now in `engine.ts`.

**Gap:** `PARTIAL` — the two devices are independently selectable, which is what
the entry asked for. What is still divergent is the _shape_: FSP8 splits the
menu by platform because the driver model does, and MotionLab has one preference
whose availability depends on the browser rather than on the operating system.
That is a difference worth stating in an ADR, not a gap to close.
...
(`src/components/browser/`) driven by `Engine.audition(mediaId)`
(`src/audio/engine.ts`), but it plays through the same single destination as
everything else — there is no separate audition output, because there is no
output channel list. This entry said the word appeared nowhere in `src/`, which
has not been true since the browser gained preview; what is absent is the
_channel_, not the function.

**Gap:** `MISSING` — for the separate output. AD-3 is closed, so this is now
implementable: `setSinkId` is per-`AudioContext`, and a second context is what
an audition bus with its own output would need.
...
| ------ | ------------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| AX-E1  | **Enable Crosshair Cursor for Tools**       | engaged    | "a large, white, vertical-and-horizontal crosshair in the Arrange view that aids in displaying the exact position of the various mouse tools"                                                                                                                                                                                                       | Absent. `uiStore.tool: ArrangeTool` exists; no crosshair overlay. This row used to claim the word appeared nowhere in `src/`; it appears three times in `src/styles/`, every one of them the CSS `cursor` keyword — a pointer shape, not the full-height guide the manual describes. The verdict stands on the overlay, which is what was actually looked for | `MISSING`             |
