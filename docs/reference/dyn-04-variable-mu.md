...
* **LATERAL/VERTICAL**: a sum/difference matrix is formed *in the transformers themselves*. The input
...
control node, so the *observed* release is the superposition of a fast branch that discharges quickly
...
2. **The 6386 push-pull gain stage — the dominant nonlinearity.** Two mechanisms superimpose:
   * *Push-pull cancellation of even harmonics.* A balanced push-pull stage cancels second-order
   ...
   * *Bias-dependent curvature.* As the sidechain drives the grids toward cutoff, the operating point
...
8. **Threshold control sense.** Assert that *decreasing* the THRESHOLD control value increases gain
...
*Position 6, individual-peak release.* The manual's full text as archived gives **0.3 s**. A widely
...
*Whether positions 5 and 6 are "automatic release".* Some sources call them auto-release settings.
...
*Ratio.* Sources give 2:1 to 30:1 and also "1:2 to 1:30" (the same range written the other way round).
...
*Meter switch positions.* Not established. **Unknown** — recorded as such rather than guessed.
