...
The whole EQ network sits *between* the input transformer and the valve amplifier. There is no
...
**The interaction that defines the unit.** Boost and attenuation on the low band are *not* inverse
...
low-frequency behaviour is *unpredictable* rather than cleanly parameterisable. **Implementer rule:**
...
Behavioural characteristics that a textbook biquad will *not* reproduce:
...
1. **Input transformer, 600 Ω.** Nonlinear at low frequencies and at high levels. Two distinct
   ...
   hysteresis, which produces distortion at *low* signal levels and never goes away, and core
   saturation, which produces distortion at *high* levels. The distortion produced is predominantly
...
5. **Low-end trick, the critical test.** LOW FREQ = 60 Hz, LOW BOOST = maximum, LOW ATTEN = maximum.
   ...
   response is *not* flat. Any model that returns a flat line here has summed the legs in dB and is
...
*Low and high band maxima.* Hardware retail listings and the original manual give +13.5/-17.5 dB low
...
*Push-pull versus single-ended make-up amplifier.* One retail description says push-pull; the tube
...
*Maximum output.* The manual's +21 dBm is used. A +24 dBm figure appears in some secondary writing
and is not supported by the manual.

*Whether simultaneous low boost and cut is "supposed" to work.* The manual says no, practice says yes,
