...
sidechain only, and swapping them changes the *dynamics*, not the tone. **Implementer rule: the
...
combined threshold-and-ratio control: turning it up lowers the effective threshold *and* steepens the
...
interact with the detector because the detector taps the output *before* — see the caveat below.
...
* **The photocell's own frequency dependence.** The published specification for the compression ratio
  ...
  *dynamics*, not in the steady-state response.
...
6. **The sidechain, inside the loop.** The 6AQ5 driving the EL panel distorts the control voltage.
   ...
   audible effect is on the *shape of the curve*, not on the spectrum. The feedback loop partially
...
12. **Make-up gain isolation.** At a fixed PEAK REDUCTION producing 10 dB of reduction, sweep GAIN
    across its full range. Assert the *amount* of gain reduction changes by less than 1 dB. If it
...
* GroupDIY threads on LA-2A topology and the T4 cell (groupdiy.com threads 50129, 80210, 18618,
  86194) — feedback topology with the sidechain picking up after the opto cell, sidechain comprising
...
*Release stage two: 0.5–5 s or 1–15 s?* The manufacturer's own specification sheet says 0.5 to 5 s
...
*Ratio in LIMIT: ∞:1 or 10–20:1?* The manufacturer says approximately ∞:1; measurement-based
...
*Ratio in COMPRESS: is 3:1 a ratio at all?* No. Every source that gives it also says the ratio is
